# Dharitri events notifier

The notifier service is a component that receives block events synchronously
from dharitri observer nodes, and it forwards them to a subscribing component
...
these [docs](https://docs.dharitri.com/integrators/observing-squad/) 
...
can be found [here](https://github.com/TerraDharitri/drt-go-chain/blob/master/cmd/node/config/external.toml).
...
The main config file can be found [here](https://github.com/TerraDharitri/drt-go-chain-notifier/blob/main/cmd/notifier/config/config.toml).
...
[`api.toml`](https://github.com/TerraDharitri/drt-go-chain-notifier/blob/main/cmd/notifier/config/api.toml) config file for events path.
For example:
```toml
[APIPackages.events]
    Routes = [
...
- Protocol based events, such as `DCDTTransfer` or `NFTCreate`
...
Note: 
- The address field is `bech32` encoded with the tag `drt`.
...
[constants](https://github.com/TerraDharitri/drt-go-chain-notifier/blob/main/common/constants.go). Below there is the event type together with the associated marshalled data type.
- `all_events`
```json
{
  "hash": "blockHash1",
